...
class Spider1Spider(scrapy.Spider):
    ...
    pipeline = set([
        ...
        pipelines.MongoDBBigdataPipeline,
    ...
    def __init__(self):
        ...
        self.strWhere = urllib.quote( "(TI='大数据' or TI='海量数据' or TI='大规模数据')")
    ...
    def start_requests(self):
        ...
        yield scrapy.FormRequest("http://epub.sipo.gov.cn/patentoutline.action",
                                 formdata={'showType':'1','strSources':'pip','strWhere':"(TI='大数据' or TI='海量数据' or TI='大规模数据')",
                                           'numSortMethod':'0','strLicenseCode':'','numIp':'2016','numIpc':'','numIg':'218','numIgc':'1',
                                           'numIgd':'','numUg':'166','numUgc':'','numUgd':'','numDg':'7','numDgc':'','pageSize':'3',
                                           'pageNow':'1'},
    ...
    def post_over(self, response):
        ...
        for i in xrange(1, int(self.pageNumber) + 1):
            print str(self.pageNumber)
            print "req bigdata..."
            yield scrapy.FormRequest("http://epub.sipo.gov.cn/patentoutline.action",
                                     formdata={'showType': '1', 'strSources': 'pip',
                                               'strWhere': "(TI='大数据' or TI='海量数据' or TI='大规模数据')",
                                               'numSortMethod': '0', 'strLicenseCode': '', 'numIp': '2016',
                                               'numIpc': '', 'numIg': '218', 'numIgc': '1',
                                               'numIgd': '', 'numUg': '166', 'numUgc': '', 'numUgd': '', 'numDg': '7',
                                               'numDgc': '', 'pageSize': '3',
                                               'pageNow': str(i)},
                                     meta={'type': self.ptype,
                                           'page': str(i)},
    ...
    def post_detail(self, response):
        ...
        if len(patents) == 0:
            ...
            # time.sleep(3600)
            logging.error(
                "this page was blocked, try to read again. page number is " + page + ", type is " + self.ptype)
            # time.sleep(120)
            scrapy.FormRequest(
                "http://epub.sipo.gov.cn/patentoutline.action",
                formdata={'showType': '1', 'strSources': 'pip', 'strWhere': "(TI='大数据' or TI='海量数据' or TI='大规模数据')",
                          'numSortMethod': '0', 'strLicenseCode': '', 'numIp': '2016', 'numIpc': '', 'numIg': '218',
                          'numIgc': '1',
                          'numIgd': '', 'numUg': '166', 'numUgc': '', 'numUgd': '', 'numDg': '7', 'numDgc': '',
                          'pageSize': '3',
                          'pageNow': str(page)},
                dont_filter=True,
                meta={'type': self.ptype,
        ...
        for box in patents:
            ...
            item['authId'] = box.xpath(
                '''div[@class="cp_linr"]/ul/li[@class="wl228"][1]/text()''').extract()[0].replace(u'申请公布号：',
                                                                                                  '')  # 授权公告号
            # authDate  授权日
